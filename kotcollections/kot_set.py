...
class KotSet(Generic[T]):
    ...
    def _add_with_type_check(self, element: T) -> None:
        ...
        if element is not None and self._element_type is not None:
            if isinstance(element, KotSet) and self._element_type == KotSet:
                ...
                raise TypeError(f"All elements must be of type {self._element_type.__name__}, got {type(element).__name__}")
            else:
                self._elements.add(element)
    ...
    def group_by(self, key_selector: Callable[[T], R]) -> dict[R, 'KotSet[T]']:
    ...
    def associate_by(self, key_selector: Callable[[T], R]) -> dict[R, T]:
        """Returns a Map with keys generated by key_selector and values from elements."""
        return {key_selector(element): element for element in self._elements}
    ...
    def to_sorted_list(self, key: Optional[Callable[[T], Any]] = None, reverse: bool = False) -> List[T]:
