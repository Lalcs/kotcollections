...
KotList is a Python library that fully reproduces Kotlin's List interface. It brings Kotlin's rich collection operations to Python developers.
...
KotList implements runtime type checking similar to Kotlin's type system. Once a list is created with elements of a specific type, it can only contain elements of that same type.
